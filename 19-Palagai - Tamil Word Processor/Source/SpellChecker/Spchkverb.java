...
public class Spchkverb extends JFrame implements ActionListener
{
	...
	public static boolean check_eerkal_aarkal(byte[] bstr,int blen)
	{
		...
		ByteMeth me=new ByteMeth();
		if(me.endswith(bstr,eerkal)||me.endswith(bstr,aarkal))
		{
	...
	public static boolean check_aal_aan_aar_yen_om_eer(byte[] bstr,int blen)
	{
		...
		if(blen < aal.length)
			return false;
		if(me.endswith(bstr,aal)||me.endswith(bstr,eer)||me.endswith(bstr,aan)||me.endswith(bstr,aar)||me.endswith(bstr,yen)||me.endswith(bstr,om))
		{
			text2.setText("str is correct");
			return true;
		}
	...
	public static boolean check_kiru_kindru(byte[] bstr,int blen)
		{
			...
			if(blen < kiru.length)
				return false;
			if(me.endswith(bstr,kiru)||me.endswith(bstr,kindru))
			{
				text2.setText("str is correct");
				return true;
			}
	...
	public static boolean check_pattern_kiru(byte[] bstr)
		{
			...
			for(int t=0;t<blen-1;t++)
				bstr1[t]=bstr[t];
			if(me.endswith(bstr,ru)||me.endswith(bstr,kir)||me.endswith(bstr1,ki))
			{
				/*byte rem[] = me.subarray(bstr,0,blen-3);
				System.out.println("rem hai******:" + tabconvert2.revert(rem));*/
				//suggest= tabconvert2.revert(me.addarray(rem,kiru));
				tempbstr=me.addarray(bstr,u);
				return(true);
			}
			else
				return(false);
	...
	public static boolean check_pattern_kindru(byte[] bstring)
		{
			...
			byte end[] = me.subarray(bstring,blen-len,blen);
			for(int t=0;t<blen-1;t++)
				bstr1[t]=bstring[t];
			for(int t=0;t<blen-2;t++)
				bstr2[t]=bstring[t];
			System.out.println("string before u added :" + tabconvert2.revert(bstring));
			if(me.endswith(bstring,indru)||me.endswith(bstr2,ki)||me.endswith(bstr1,kin)||me.endswith(bstring,indr))
				{
					tempbstr=me.addarray(bstring,u);
					System.out.println("u added string :" + tabconvert2.revert(tempbstr));
					/*if(me.endswith(bstr1,kin))
					{
						bstr=me.addarray(bstr,u);
						System.out.println("u added string :" + tabconvert2.revert(bstr));
					}*/
						return(true);
				}
			if(me.endswith(end,r1)&&me.startswith(end,kk))
			{
				tempbstr=me.addarray(bstring,u);
	...
	/*public static boolean check_pattern_eer(byte[] bstr)
		{
			...
			int len = eer.length;
			byte ee[]={4},rr[]={25};
			byte end[] = me.subarray(bstr,blen-len,blen),end1[]={end[0]},end2[]={end[1]};
			if(me.isequal(end1,ee) || me.endswith(end2,rr))
				return(true);
			else
				return(false);
	...
	public static boolean check_pattern_yen(byte[] bstr)
	{
		...
		byte ye[]={8},nn[]={31};
		byte end[] = me.subarray(bstr,blen-len,blen),end1[]={end[0]},end2[]={end[1]};
		if(me.isequal(end1,ye) || me.endswith(end2,nn))
			return(true);
		else
			return(false);
	...
	public static boolean check_pattern_aal_aan_aar_yen_om_eer(byte[] bstr)
	{
		...
		if(me.isequal(end1,aa)||(me.endswith(end1,a)&&(me.endswith(bstr,nn1)||me.endswith(bstr,ll1)||me.endswith(bstr,ll))))  // checking all,aan,aar
		{
			if(me.endswith(bstr,ll1)||me.endswith(bstr,ll))
			{
				get_sugg(bstr,aal);
				set_sugg(bstr,aan);
				set_sugg(bstr,aar);
		...
		else if(me.endswith(bstr,nn))  // checking aan,yen
		{
			if(me.isequal(end1,A))
			{
				get_sugg(bstr,yen);
				set_sugg(bstr,aan);
		...
		else if(me.isequal(end1,O)||me.endswith(bstr,mm))
		{
			get_sugg(bstr,om);
			set_sugg(bstr,aan);
			set_sugg(bstr,yen);
		...
		else if(me.isequal(end1,E)||me.endswith(bstr,rr))
		{
			get_sugg(bstr,eer);
			set_sugg(bstr,aar);
			set_sugg(bstr,aal);
	...
	public static boolean check_pattern_athu(byte[] bstr)
	{
		...
		byte end[] = me.subarray(bstr,blen-len,blen),end23[]={end[1],end[2]},end12[]={end[0],end[1]};
		if(me.isequal(end12,ath) || me.endswith(end23,thu))
			return(true);
		else
			return(false);
	...
	public static boolean check_pattern_ana(byte[] bstr)
	{
		...
		byte end[] = me.subarray(bstr,blen-len,blen),end23[]={end[1],end[2]},end12[]={end[0],end[1]},end1[]={end[0]};
		if(me.isequal(end12,an)||me.endswith(end23,na)||me.endswith(end1,a))
			return(true);
		else
			return(false);
	...
	public static boolean check_pattern_eerkal(byte[] bstr)
		{
			...
			if(blen < len)
				return false;
			byte kk[]={14} ;
			byte end[] = me.subarray(bstr,blen-len,blen),end2[]={end[2]};
			...
			if(me.isequal(end12,eer)||me.isequal(end345,kal) || me.isequal(end234,rka))
				return(true);
			else
				return(false);
	...
	public static boolean check_pattern_aarkal(byte[] bstr)
	{
		...
		byte end[] = me.subarray(bstr,blen-len,blen),end2[]={end[2]};
		byte end12[] = {end[0],end[1]};byte end345[] = {end[2],end[3],end[4]};byte end234[] = {end[1],end[2],end[3]};
		if(/*me.isequal(end2,kk) || */me.isequal(end12,aar) || me.isequal(end345,kal) || me.isequal(end234,rka))
			return(true);
		else
			return(false);
